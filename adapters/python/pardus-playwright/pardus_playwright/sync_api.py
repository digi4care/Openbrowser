"""Synchronous Playwright-compatible API backed by PardusBrowser."""
...
def _find_pardus_browser() -> str:
    for candidate in [
        os.environ.get("PARDUS_BROWSER_PATH"),
        "pardus-browser",
    ]:
    ...
    raise FileNotFoundError(
        "pardus-browser not found. Install PardusBrowser or set PARDUS_BROWSER_PATH."
    )
...
class PardusLauncher:
    def __init__(
        ...
        self.binary_path = binary_path or _find_pardus_browser()
    ...
    def start(self) -> str:
        ...
        while time.time() < deadline:
            if self._process.poll() is not None:
                ...
                self._process = None
                raise RuntimeError(f"pardus-browser exited early: {stderr}")
            try:
        ...
        self.stop()
        raise TimeoutError(
            f"pardus-browser did not start within {self.timeout}s"
        )
...
@contextmanager
def sync_playwright():
    """Synchronous Playwright context manager that uses PardusBrowser.
    ...
    try:
        with _native_sync_playwright() as native_p:
            ...
            class PardusPlaywrightContext:
                def __init__(self, native, _launcher):
                    self.chromium = _PardusBrowserType(native.chromium, _launcher)
            ...
            yield PardusPlaywrightContext(native_p, launcher)
    finally:
        launcher.stop()


class _PardusBrowserType:
    def __init__(self, native_type, launcher: PardusLauncher):
    ...
    def launch(self, **kwargs):
        ...
        if not cdp_url:
            raise RuntimeError("PardusBrowser is not running")

        browser = self._native.connect_over_cdp(cdp_url)
        browser._pardus_launcher = self._launcher
        return browser
